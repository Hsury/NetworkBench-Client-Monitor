Public Class Form1
    ...
    Dim Initial, S1, S2, S3, S4, S5, S6, S7, S8, S9, S10, S11, S12, S13, S14, S15 As Integer
    ...
    Private Sub WriteLog()
        ...
        If TextBox1.Text = "" Then
            TextBox1.Text = Format(TimeNow, "HH:mm") & " " & Log & TextBox1.Text
        Else
            TextBox1.Text = Format(TimeNow, "HH:mm") & " " & Log & vbCrLf & TextBox1.Text
        End If
        IO.Directory.CreateDirectory("NBMonitorLog")
        ...
        If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\status") = 5 Then
            ...
            Else
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\rt") > NBRT Then
                    Log = "[任务] 请求任务数+" & WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\rt") - NBRT
                    WriteLog()
                    NBRT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\rt")
                    Label6.Text = "请求任务数:" & NBRT
                    S3 = 1
                ...
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\dt") > NBDT Then
                    Log = "[任务] 完成任务数+" & WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\dt") - NBDT
                    WriteLog()
                    NBDT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\dt")
                    Label7.Text = "完成任务数:" & NBDT
                    S3 = 1
                ...
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\ut") > NBUT Then
                    Log = "[任务] 上传任务数+" & WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\ut") - NBUT
                    WriteLog()
                    NBUT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\ut")
                    Label8.Text = "上传任务数:" & NBUT
                    S3 = 1
                ...
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\wt") > NBWT Then
                    Log = "[任务] 等待任务数+" & WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\wt") - NBWT
                    WriteLog()
                    NBWT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\wt")
                    Label9.Text = "等待任务数:" & NBWT
                    S3 = 1
                End If
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\wt") < NBWT Then
                    Log = "[任务] 等待任务数-" & NBWT - WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\wt")
                    WriteLog()
                    NBWT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\wt")
                    Label9.Text = "等待任务数:" & NBWT
                    S3 = 1
                ...
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\et") > NBET Then
                    Log = "[任务] 超时任务数+" & WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\et") - NBET
                    WriteLog()
                    NBET = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\et")
                    Label10.Text = "超时任务数:" & NBET
                    S3 = 1
                ...
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft") > NBUFT Then
                    Log = "[任务] 上传失败任务数+" & WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft") - NBUFT
                    WriteLog()
                    NBUFT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft")
                    Label11.Text = "上传失败任务数:" & NBUFT
                    S3 = 1
                End If
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft") < NBUFT Then
                    Log = "[任务] 上传失败任务数-" & NBUFT - WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft")
                    WriteLog()
                    NBUFT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft")
                    Label11.Text = "上传失败任务数:" & NBUFT
                    S3 = 1
...
Err:
    ...
    Private Sub GetNBData()
        If NBRunTime <> 0 Then
            If Label25.Text <> Math.Round(NBDT / (NBRunTime / 3600)) & " 个/小时" Then
                Label25.Text = Math.Round(NBDT / (NBRunTime / 3600)) & " 个/小时"
                Label26.Text = Math.Round(NBDT / (NBRunTime / 86400)) & " 个/天"
                Log = "[统计] 任务执行速度:" & Math.Round(NBDT / (NBRunTime / 3600)) & "个/小时 = " & Math.Round(NBDT / (NBRunTime / 86400)) & "个/天"
                WriteLog()
            End If
            If Label27.Text <> "状态异常次数:" & ErrTime Then
                Label27.Text = "状态异常次数:" & ErrTime
                Log = "[统计] 状态异常次数:" & ErrTime
                WriteLog()
            End If
        Else
            Label25.Text = "0 个/小时"
            Label26.Text = "0 个/天"
            Label27.Text = "状态异常次数:0"
        End If
    End Sub
...
Err:
    ...
    Private Sub Timer3_Tick(sender As Object, e As EventArgs) Handles Timer3.Tick
        If CheckBox3.Checked = True Then
            If S14 = NBRT And S15 = NBDT Then
                Log = "[服务] 长时间无任务,即将重启服务"
                WriteLog()
                ServiceController1.Stop()
                ServiceController1.Start()
                Log = "[服务] 服务已重启"
...
Err:
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Shell("notepad.exe " & "NBMonitorLog\" & Format(TimeNow, "yyyy年M月d日") & ".txt", vbNormalFocus)
    End Sub
...
Err:
    ...
    Private Sub CheckBox3_CheckedChanged(sender As Object, e As EventArgs) Handles CheckBox3.CheckedChanged
        If S13 = 1 Then
            ...
            If CheckBox3.Checked = True Then
                Log = "[控制] 开启了长时间无任务时自动重启的功能"
                WriteLog()
                S14 = NBRT
                S15 = NBDT
                Timer3.Enabled = True
            Else
                Log = "[控制] 关闭了长时间无任务时自动重启的功能"
                WriteLog()
            End If
        End If
    End Sub
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        NoErr = 1
        Log = "[信息] 欢迎使用基调网络监测客户端状态监控工具"
        WriteLog()
        Timer1.Enabled = True
    End Sub

    Private Sub ErrControl()
        Dim Choose As Integer
        If NoErr = 0 And Initial < 7 Then
            Me.Hide()
            Timer1.Enabled = False
            Timer2.Enabled = False
            Log = "[信息] 程序由于某些错误无法运行"
            WriteLog()
            MsgBox("程序由于某些错误无法运行。" & vbCrLf & vbCrLf & "您可以从以下几个方面进行检查：" & vbCrLf & "（1）确认本机已正确安装基调网络监测客户端" & vbCrLf & "（2）以管理员身份运行本程序" & vbCrLf & "（3）将本程序升级到最新版本", vbCritical, "基调网络监测客户端状态监控工具")
            Choose = MsgBox("您是否愿意前往软件发布页面？" & vbCrLf & vbCrLf & "在这里您可以：" & vbCrLf & "（1）获取本程序的最新版本" & vbCrLf & "（2）向作者反馈您遇到的问题", vbYesNo + vbQuestion, "基调网络监测客户端状态监控工具")
            If Choose = 6 Then Shell("explorer.exe http://www.kagamiz.com/nbmonitor/", vbNormalFocus)
            Me.Close()
        End If
    End Sub
End Class
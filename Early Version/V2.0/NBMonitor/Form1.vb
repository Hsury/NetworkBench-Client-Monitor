...
Err:
    ...
    Private Sub GetNBData()
        If NBRunTime <> 0 Then
            Label24.Text = "每分钟可完成任务数:" & Math.Round(NBDT / (NBRunTime / 60))
            Label25.Text = "每小时可完成任务数:" & Math.Round(NBDT / (NBRunTime / 3600))
            Label26.Text = "每天可完成任务数:" & Math.Round(NBDT / (NBRunTime / 86400))
        Else
...
Err:
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        Shell("explorer.exe " & "NBMonitorLog\" & Format(TimeNow, "yyyy年M月d日") & ".txt", vbNormalFocus)
    End Sub
...
Err:
    ...
    Private Sub ComboBox1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ComboBox1.SelectedIndexChanged
        If S13 = 1 Then
            Timer2.Interval = Val(ComboBox1.Text) * 1000 * 60
            Log = "[控制台] 状态刷新间隔调整为" & ComboBox1.Text & "分钟"
            WriteLog()
        End If
    End Sub
    ...
    Private Sub ErrControl()
        ...
        If NoErr = 0 Then
            ...
            Timer1.Enabled = False
            Timer2.Enabled = False
            Log = "[信息] 程序由于某些错误无法正常运行"
            WriteLog()
            MsgBox("程序由于某些错误无法正常运行。" & vbCrLf & vbCrLf & "您可以从以下几个方面进行检查：" & vbCrLf & "（1）以管理员身份运行运行本程序" & vbCrLf & "（2）确认本机已正确安装基调网络监测客户端" & vbCrLf & "（3）将本程序升级到最新版本", vbCritical, "基调网络监测客户端状态监控工具")

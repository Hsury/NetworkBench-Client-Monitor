Public Class Form1
    ...
    Private Sub WriteLog()
        Dim TimeNow As Date
        TimeNow = Now
        TextBox1.Text = TimeNow & " " & Log & vbCrLf & TextBox1.Text
        IO.File.AppendAllText("NBMonitor_Logs\" & Format(Date.Today, "yyyy-MM-dd") & ".txt", TimeNow & " " & Log & vbCrLf)
    End Sub
    ...
    Private Sub GetNBService()
        ...
        If NBStartTime <> "" Then
            ...
            Label4.Text = "运行时长:" & Int(NBRunTime / 86400) & "天" & Int((NBRunTime - Int(NBRunTime / 86400) * 86400) / 3600) & "小时" & Int((NBRunTime - Int(NBRunTime / 3600) * 3600) / 60) & "分" & NBRunTime - Int(NBRunTime / 60) * 60 & "秒"
            Button1.Text = "停止任务"
            If S1 <> 1 Then
                Log = "[服务] 检测到服务已启动"
                WriteLog()
                ...
                WriteLog()
                Log = "[服务] 已运行时长:" & Int(NBRunTime / 86400) & "天" & Int((NBRunTime - Int(NBRunTime / 86400) * 86400) / 3600) & "小时" & Int((NBRunTime - Int(NBRunTime / 3600) * 3600) / 60) & "分" & NBRunTime - Int(NBRunTime / 60) * 60 & "秒"
                WriteLog()
                S1 = 1
            End If
        Else
    ...
    Private Sub GetNBTask()
        If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\status") = 5 Then
            If S2 <> 1 Then
                NBRT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\rt") '请求任务
                NBET = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\et") '超时任务
                NBDT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\dt") '完成任务
                NBWT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\wt") '等待任务
                NBUT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\ut") '上传任务
                NBUFT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft") '上传失败
                Label5.ForeColor = Color.LimeGreen
                Label5.Text = "正在执行任务"
                Label6.Text = "请求任务数量:" & NBRT
                Label7.Text = "超时任务数量:" & NBET
                Label8.Text = "完成任务数量:" & NBDT
                Label9.Text = "等待任务数量:" & NBWT
                Label10.Text = "上传任务数量:" & NBUT
                Label11.Text = "上传失败任务数量:" & NBUFT
                Log = "[任务] 正在执行任务"
                WriteLog()
            ...
            Else
                ...
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft") > NBUFT Then
                    ...
                    WriteLog()
                    NBUFT = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\ut")
                    Label11.Text = "上传失败任务数量:" & NBUFT
                    S3 = 1
                End If
                If WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\uft") < NBUFT Then
    ...
    Private Sub GetNBStatus()
        NBsConns = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\sConns") 'Conns状态
        NBsCPU = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\sCPU") 'CPU状态
        NBsDisk = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\sDisk") '硬盘状态
        NBsMem = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\sMem") '内存状态
        NBsPageFile = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\sPageFile") '页面文件状态
        NBsPing = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\sPing") '网络延迟状态
        If NBsConns Then
            Label13.ForeColor = Color.Orange
            Label13.Text = "Conns状态:异常"
            S4 = 0
            If S5 = 0 Then
                Log = "[系统] Conns状态出现异常"
                WriteLog()
        ...
        Else
            ...
            End If
        End If
        If NBsConns Or NBsCPU Or NBsDisk Or NBsMem Or NBsPageFile Or NBsPing Then
    ...
    Private Sub GetNBNetwork()
        If (NBCity <> WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\City")) Or (NBISP <> WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\Isp")) Or (NBSpeed <> WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\Speed")) Then
            S11 = 1
        End If
        NBCity = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\City") '城市代码
        NBISP = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\Isp") '运营商代码
        NBSpeed = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\Speed") '网速代码
        ...
        End Select
        Select Case NBSpeed
            Case 5 : Label23.Text = "连接速度:普通MODEM或ISDN"
            Case 6 : Label23.Text = "连接速度:ADSL或小区宽带"
            Case 7 : Label23.Text = "连接速度:企业专线"
            Case 8 : Label23.Text = "连接速度:IDC机房专线"
            Case Else : Label23.Text = "连接速度:未知"
        End Select
        If S11 = 1 Then
    ...
    Private Sub GetNBData()
        If NBRunTime <> 0 Then
            Label24.Text = "每分钟可完成任务数:" & Math.Round(NBDT / (NBRunTime / 60), 2)
            Label25.Text = "每小时可完成任务数:" & Math.Round(NBDT / (NBRunTime / 3600), 2)
            Label26.Text = "每天可完成任务数:" & Math.Round(NBDT / (NBRunTime / 86400), 2)
        Else
    ...
    Private Sub GetNBConfig()
        NBAutoLogin = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\AutoLogin") '自动登录
        NBAutoStart = WshShell.RegRead("HKEY_LOCAL_MACHINE\SOFTWARE\NetworkBench\AutoStart") '隐藏图标
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Select Case Initial
            ...
            Case 7
                GetNBConsole()
                If DateDiff("d", "2015/06/28", Date.Today) > 7 Then
                    Timer1.Enabled = False
                    MsgBox("NBMonitor V1.0生命周期已终结!" & vbCrLf & "请联系作者(QQ:2726142145)来获取更新.", vbOKOnly + vbExclamation)
                    Me.Close()
                End If
                Timer1.Enabled = False
                Timer2.Enabled = True
    ...
    Private Sub Button4_Click(sender As Object, e As EventArgs) Handles Button4.Click
        Shell("explorer.exe " & "NBMonitor_Logs\" & Format(Date.Today, "yyyy-MM-dd") & ".txt", vbNormalFocus)
    End Sub
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Log = "欢迎使用 基调网络监测客户端状态监控工具 V1.0"
        WriteLog()
        Log = "此版本编译于:2015年6月28日 17:30"
        WriteLog()
        Timer1.Enabled = True
    End Sub

    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Select Case Initial
            ...
            Case 7
                ...
                GetNBConsole()
                If DateDiff("d", "2015/07/21", Date.Today) > 31 Then
                    NoErr = 0
                    ErrControl()
                End If
                Timer2.Enabled = True
...
Err:
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        NoErr = 1
        Log = "[信息] 欢迎使用 基调网络监测客户端状态监控工具 V1.5"
        WriteLog()
        Log = "[信息] 此版本编译于:2015年7月21日 16:00"
        WriteLog()
        Timer1.Enabled = True
    End Sub
    ...
    Private Sub ErrControl()
        ...
        If NoErr = 0 Then
            ...
            WriteLog()
            MsgBox("程序由于某些错误无法正常运行。" & vbCrLf & vbCrLf & "您可以从以下几个方面进行检查：" & vbCrLf & "（1）以管理员身份运行运行本程序" & vbCrLf & "（2）确认本机已正确安装基调网络监测客户端" & vbCrLf & "（3）将程序升级到最新版本", vbCritical, "基调网络监测客户端状态监控工具")
            Choose = MsgBox("您是否愿意前往软件发布页？" & vbCrLf & vbCrLf & "在这里您可以：" & vbCrLf & "（1）获取最新版本的程序" & vbCrLf & "（2）向作者反馈您遇到的问题", vbYesNo + vbQuestion, "基调网络监测客户端状态监控工具")
